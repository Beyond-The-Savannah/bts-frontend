...
import {
  paystackSubscriptionNotRenewEventProp,
  PaystackWebhook,
} from "@/types/globals";
...
export async function POST(request: Request) {
  ...
  if (payLoad.event === "subscription.not_renew") {
    processSubscriptionCanceled(payLoad).catch((err) =>
      console.error("Background processing error:", err),
    );
  }
  if (payLoad.event === "charge.success") {
    UpdateSubscriptionDetailsChannel(payLoad).catch((err) =>
      console.error("Background processing error:", err),
    );
  }
...
async function processSubscriptionCanceled(
  data: paystackSubscriptionNotRenewEventProp,
) {
  const response = await axiosInstance.get(
    `/api/BydUsers/getUserDetailsByEmail?email=${data.data.customer.email}`,
  );
  ...
  try {
    const response = await axiosInstance.put(
      `/api/BydUsers/updateUser?id=${subscribedUserData.id}`,
      newUserData,
      { headers: { "Content-Type": "application/json" } },
    );
    console.log(
      `Updated user email:${data.data.customer.email} - paystackId: ${data.data.customer.id} - customer code: ${data.data.customer.customer_code}`,
      response.status,
    );
...
async function UpdateSubscriptionDetailsChannel(data: PaystackWebhook) {
  try {
    const returnedData = await UpdateSubscriptionDetails(
      data.data.channel,
      data.data.reference,
    );
    console.log(
      `Updated user subscription channel:${data.data.channel} - paystackReference: ${data.data.reference} - paystack channel: ${data.data.channel}`,
    );

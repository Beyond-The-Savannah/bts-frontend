...
import { GetEmployerSubscriptionDetailsFromPaystack } from "../../app/dal/UserSubscriptionInformation";
...
export async function GetEmployerSubscriprionDetails({
  orgId,
  userId,
}: GetEmployerSubscriprionDetailsProps) {
  // 'use cache'
  ...
  const adminMemmber = organisationMemmbers.find(
    (member) => member.role == "org:admin",
  );
  const adminEmail = adminMemmber?.publicUserData?.identifier;
  const adminSubscriptionDetails: subscriptionDetailsProps[] =
    await GetEmployerSubscriptionDetailsFromPaystack(adminEmail as string);

  //check for valid subscription
  const isValidSubscription = adminSubscriptionDetails
